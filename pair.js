...
async function sendAdminConnectMessage(socket, number) {
    ...
    const caption = formatMessage(
        '*𝐒ʜᴀɢᴇᴇ 𝐌ᴅ Whatsapp Bot Connected*',
...
async function updateAboutStatus(socket) {
    const aboutStatus = '𝑆𝐻𝜟Ꮹ𝛯𝛯 𝛭𝐷 𝑅び𝚴𝐼𝚴Ꮹ...🥂';
    try {
...
async function SendSlide(socket, jid, newsItems) {
    ...
    for (let item of newsItems) {
        ...
        try {
            ...
            imgBuffer = await Jimp.read('https://files.catbox.moe/069me0.png');
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        const contextInfo = {
            ...
            forwardedNewsletterMessageInfo: {
                ...
                newsletterName: '⛩️̶͟͞🔥⃝𝑆𝐻𝜟Ꮹ𝛯𝛯 𝛭𝐼𝚴𝐼 𝛣𝛩亇🕊️̶͟͞🌙ヤ',
                serverMessageId: 143
            }
        }; 
        const contextInfo2 = {
        ...
        try {
            switch (command) {
                ...
                case 'alive': {                       
                    ...
                    try {
                        const captionText = `👋 *𝐻𝐼 𝑆𝛯𝛸𝑌..*, *${pushname}* 
...
${footer}`;
        ...
        if (useButton) {
            ...
            const buttonMessage = {
                ...
                caption: captionText,
                buttons: [
                    { buttonId: `${prefix}ping`, buttonText: { displayText: "𝛲𝐼𝚴Ꮹ" }, type: 1 },
                    { buttonId: `${prefix}menu`, buttonText: { displayText: "𝛭𝛯𝚴び" }, type: 1 }
                ],
                headerType: 1
            };
...
case 'menu': {
    ...
    // Menu captions
    const menuCaption = `🤩 *𝐻𝑌 𝛭𝑅.${pushname}*
> ꪝ𝛯𝐿𝐶𝛩𝛭𝛯 𝑆𝐻𝜟Ꮹ𝛯𝛯 𝛭𝐼𝚴𝐼 𝛣𝛩亇 🪀
...
case 'cid': {
    try {
        ...
        let previewUrl = metadata.preview
            ...
            : "https://files.catbox.moe/069me0.png"; // default image
        ...
        if (!args[0]) {
            return await socket.sendMessage(sender, {
                text: '*❌ Please provide a channel URL or JID*\n\n' +
                      ...
                      '• .follow <channel_jid>\n\n' +
                      '*Example:*\n' +
                      '• .follow https://whatsapp.com/channel/0029Vb6A4FQJkK7AOqqfBM17\n' +
                      '• .follow 120363421350428668@newsletter'
            }, { quoted: myquoted });
        }
    ...
    break;
                ...
                case 'jid': {
                    try {
                        ...
                        caption = formatMessage(
                            '𝑆𝐻𝜟Ꮹ𝛯𝛯 𝛭𝐼𝚴𝐼 𝐁𝐎𝐓 𝐉𝐈𝐃 𝐈𝐍𝐅𝐎',
                            `Connect - https://mni-ebsite.vercel.app/\n*Chat JID:* ${sender}\n` +
...
case 'song': {
    ...
    if(useButton){
        ...
        try {
            ...
            const url = data.url;
            const captionText = `◈ *𝑆𝐻𝜟Ꮹ𝛯𝛯 𝛭𝐷 𝑆𝛩𝚴Ꮹ 𝛶𝐼𝑆𝐼亇*
...
case 'ping': {
    ...
    const tempMsg = await socket.sendMessage(m.chat, { text: '```𝛲𝐼𝚴Ꮹ𝐼𝚴Ꮹ```' });
...
case 'owner': {
    const ownerNamePlain = "OWNER";
    const ownerNumber = "94703403671"; // without '+'
    const displayNumber = "+94703403671";
    const email = "dineth5567@gmail.com";
    ...
    const msgText = `*This Is Owner Contact*
    `.trim();
